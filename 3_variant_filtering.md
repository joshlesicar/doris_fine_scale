...
removed from both data sets. This was done in R, using the packages
`adegenet` and `vcfr`.
...
the inflated estimates of genetic variation in samples. This step
required extracting all SNP identifications, and extracting the first
one from each locus into a list that was then used to filter the .vcf
file.
